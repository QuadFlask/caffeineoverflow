...
public final class FlaskUtil {
	...
	public final static Color[] COLOR_FIRE_ALPHA = { new Color(0x44000000, true), new Color(0x44440000, true),new Color(0x44880000, true), new Color(0x88ff0000, true), new Color(0xccffff00, true), new Color(0xffffffff, true) };
	...
	/**
	 * 占쏙옙占쏙옙占쏙옙틱 占쌉쇽옙(S-Line)
	 * 
	 * @param $step
	 *            : logistic 占쏙옙占쏙옙 占쏙옙占쏙옙 [0, 1]
	 * @param $from
	 *            : 처占쏙옙占쏙옙
	 * @param $to
	 *            : 占쏙옙占쏙옙占쏙옙 占쏙옙
	 * @return : logistic 占쌉쇽옙 占쏙옙占쌉곤옙
	 */
	...
	/**
	 * 占쏙옙占쏙옙占쏙옙틱 占쌉쇽옙(S-Line) : (0~1) 占쏙옙占쏙옙화
	 * 
	 * @param $n
	 *            : 占쏙옙占쏙옙 占쏙옙占쏙옙
	 * @return : 占쏙옙占쏙옙 logistic占쌉쇽옙占쏙옙
	 */
	...
	/**
	 * 占쏙옙占썲러틱 占쏙옙占쏙옙占쏙옙 占쌉쇽옙
	 * 
	 * @param t
	 *            : 占쏙옙占쏙옙
	 * @param B
	 *            : 커占쏙옙 占쏙옙占쏙옙트
	 * @return : 占쏙옙占쏙옙占쏙옙 占쏘선 占쏙옙
	 */
	public final static Vec2D unit_Quadratic_Bezier(double t, Vec2D B) {
		if (t < 0 || t > 1) return null;// throw new
										// Error("t must in [0, 1] : + bezier_curve()");
	...
	/**
	 * 占쏙옙占싻울옙 占쏙옙占쏙옙 占쏙옙칭占쏙옙 占쏙옙占�
	 * 
	 * @param A
	 *            : 占쏙옙占쏙옙 l 占쏙옙 占쏙옙占쏙옙 占쏙옙
	 * @param B
	 *            : 占쏙옙占쏙옙 l 占쏙옙 占쌕몌옙占쏙옙 占쏙옙
	 * @param p
	 *            : 占쏙옙칭占쏙옙 占쏙옙
	 * @param intersection
	 *            : 占쏙옙占쏙옙占쏙옙 占쏙옙占쌀곤옙占쏙옙占쏙옙
	 * @return : 占쏙옙칭占쏙옙 占쏙옙占�
	 */
	...
	// 占쏙옙占쏙옙占쌩쏙옙 占쏙옙占싹깍옙 - Center of Mass
	 ...
	 * @param p1
	 * @param i
	 * @return : n 占쏙옙占쏙옙 占쏙옙占쏙옙 占쏙옙占쏙옙트
	 */
	...
	public final static Vec2D getIntersection(Vec2D a1, Vec2D a2, Vec2D b1, Vec2D b2) {
		...
		if (a1.x == a2.x) inter = new Vec2D(a1.x, f2 * a1.x + g2);
		else if (b1.x == b2.x) inter = new Vec2D(b1.x, f1 * b1.x + g1);
		else {
			inter = new Vec2D((g2 - g1) / (f1 - f2), 0);
			inter.y = f1 * inter.x + g2;
		}
		if (isRect(inter, a1, a2) && isRect(inter, b1, b2)) return inter;
		return null;
	}
	...
	public final static Vec2D getCircumcenter(Vec2D a, Vec2D b, Vec2D c) {
		...
		else if (a.y == b.y) return new Vec2D(m1.x, f2 * m1.x + g2);
		else if (c.y == b.y) return new Vec2D(m2.x, f1 * m2.x + g1);
	...
	public final static Vec2D[] getIntersectionCircle2Circle(Vec2D A, double r, Vec2D B, double R) {
		...
		if (d == 0) { // 占쏙옙占쏙옙 占쏙옙침 -> 占싫몌옙占쏙옙占신놂옙 占쏙옙占쏙옙占쏙옙 占쏙옙占싼곤옙
			return null;
		} else if (d > R + r) { // 占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙
			return null;
		} else if (d == R + r) { // 占쏙옙占쏙옙 1占쏙옙
			result = new Vec2D[1];
			...
			if (th1 > Math.PI / 2) th1 = Math.PI - th1;

			double th = Math.acos((B.x - A.x) / d);

			if (B.x - A.x < 0) th = Math.PI - th;
			if (B.x - A.x <= 0 && B.y - A.y >= 0) th = Math.PI - th;
			else if (B.x - A.x <= 0 && B.y - A.y <= 0) th = Math.PI + th;
			else if (B.x - A.x >= 0 && B.y - A.y <= 0) th = 2 * Math.PI - th;
	...
	// public final static Color colorInterpolate(Color from, Color to, double
	// t) {
	// if (t < 0)
	// return from;
	// else if (t >= 1)
	// return to;
	// int nr = (int) (linearInterpolate(from.getRed(), to.getRed(), t) + .5);
	// int ng = (int) (linearInterpolate(from.getGreen(), to.getGreen(), t) +
	// .5);
	// int nb = (int) (linearInterpolate(from.getBlue(), to.getBlue(), t) + .5);
	...
	public final static Color colorInterpolate(Color[] list, double t) {
		...
		else if (t >= 1) return list[list.length - 1];
		int L = list.length;
		if (L < 2) return list[0];
		float gap = 1.0f / (L - 1);
	...
	public final static String HSVtoHEX(double $H, double $S, double $V) {
		...
		default: {
			R = G = B = 0;
			throw new IllegalArgumentException("첫占쏙옙째 占식띰옙占쏙옙占�$H)占쏙옙 占쌩몌옙퓸占쏙옙占싹댐옙. : " + $H);
		}
		}
		R *= 255.0;
		G *= 255.0;
	...
	public final static int HSVtoRGB(double $H, double $S, double $V) {
		...
		default: {
			// throw new Error("첫占쏙옙째 占식띰옙占쏙옙占�$H)占쏙옙 占쌩몌옙퓸占쏙옙占싹댐옙. : " +$H);
			R = G = B = 0;
			break;
		}
		}
		...
		G *= 255.0;
		B *= 255.0;
		// System.out.println(((int) (R + .5)) + " " + ((int) (G + .5)) + " " +
		// ((int) (B + .5)));
		// (r0 << 16) | (g0 << 8) | b0;
