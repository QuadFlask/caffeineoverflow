...
import main.game.resource.BMPFont;
...
public class GraphicsDrawer {
	...
	public static void drawBitmapTextCenter(Graphics g, final String text, final int size, final int gap, final Color color, final int ix, final int iy) {
		int width = GraphicsDrawer.getTextWidth(text, size);
		drawBitmapText(g, text, size, gap, color, ix - width / 2, iy);
	}
	
	public static void drawBitmapTextRight(Graphics g, final String text, final int size, final int gap, final Color color, final int ix, final int iy) {
		int width = GraphicsDrawer.getTextWidth(text, size);
		drawBitmapText(g, text, size, gap, color, ix - width, iy);
	}
	...
	public static int getTextWidth(final String text, final int size) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (textarr != null) cursor += textarr[0].length + 1;
			else cursor += BMPFont.LETTERS.get('?').length + 1;
		}
		cursor--;
		return cursor * size;
	}
	...
	public static void drawBluckBoxesByGauge(Graphics g, final int ix, final int iy, final int sx, final int sy, final int gap, int value, int total, final Color gauge, final Color line) {
		...
		// drawBluckBoxesByVLine(g, ix - sy + gap, iy - sy - gap, sy, sy, gap,
		// 3, line);
		// drawBluckBoxesByVLine(g, ix + total * sx + gap, iy - sy - gap, sy,
		// sy, gap, 3, line);
